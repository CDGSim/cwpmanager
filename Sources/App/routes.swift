...
func routes(_ app: Application) throws {
    ...
    app.webSocket("distribution") { req, ws in
        ...
        Task {
            if let payload = try? JSONEncoder().encode(odsAMANManager.positionLayout) {
    ...
    app.get("didSetODS", ":positionName", "branch", ":branchID") { req -> HTTPStatus in
        let positionName = req.parameters.get("positionName")!
        guard let branchID = Int(req.parameters.get("branchID")!) else {
            return HTTPStatus.badRequest
        }
        
        if let positionIndex = odsAMANManager.positionLayout.controllerWorkingPositions.firstIndex(where:{ cwp in
            cwp.name == positionName
        }) {
            odsAMANManager.positionLayout.controllerWorkingPositions[positionIndex].simulationBranchNumber = branchID
            // Trigger an update, the manager will notify all its clients
            odsAMANManager.didReceivePositionLayout(odsAMANManager.positionLayout)
        }
        return HTTPStatus.ok
    }
}
